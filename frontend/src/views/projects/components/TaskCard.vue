...
<template>
  ...
  >
    <div class="pr-6">
      <p class="text-sm font-bold text-brand-text group-hover/todo:text-brand-primary transition-colors leading-tight text-left truncate">{{ todo.title }}</p>
      <p v-if="todo.details" class="text-[10px] text-brand-text-muted mt-2 line-clamp-2 leading-relaxed font-medium text-left">{{ todo.details }}</p>
    </div>
