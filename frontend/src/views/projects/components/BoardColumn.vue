...
const isVisible = (todo: Todo) => {
  if (!props.filterTags?.length) return true;
  return todo.tags?.some((t) => props.filterTags!.includes(t.pid)) ?? false;
};

const visibleCount = computed(() => {
  if (!props.filterTags?.length) return props.board.todos?.length || 0;
  return props.board.todos?.filter((t) => isVisible(t)).length || 0;
});
</script>
